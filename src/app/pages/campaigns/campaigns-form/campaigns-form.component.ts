...
export class CampaignsFormComponent implements OnInit{
    ...
    if (this.form.valid) {
        ...
        if(isDraft){
          ...
          this.confirmationService.confirm({
            message: 'After publishing your campaign you will be unable to make adjustments to your budget and rewards, also it will become publicly visible.',
  ...
  ngOnInit(): void {
    const { images, internalName, externalName, description, categories, website, link, budgetPerCreator, endDateTime, awards, status  } =
      this.request;
      ...
      website: this.fb.control(website? {id: website.id, name: website.name} : undefined, [Validators.required]),
      link: this.fb.control(link, [Validators.required]),
      budgetPerCreator: this.fb.control(budgetPerCreator, [Validators.required]),
      endDateTime: this.fb.control({value: endDateTime, disabled: this.isEditMode}, [Validators.required]),
      awards: this.fb.control({value: awards, disabled: this.isEditMode}, [Validators.required]),
